...
class RotaryEmbedding(nnx.Module):
  def __init__(
    self, dim, max_position_embeddings: int = 2048, base: int = 10000
  ):
    super().__init__()
  ...
  def _set_cos_sin_cache(self, seq_len, dtype):
    ...
    t = jnp.arange(self.max_seq_len_cached, dtype=self.inv_freq.value.dtype)

    freqs = jnp.outer(t, self.inv_freq.value)
    # Different from paper, but it uses a different permutation in order to obtain the same calculation
    emb = jnp.concatenate((freqs, freqs), axis=-1)
    self.cos_cached = Buffer(jnp.cos(emb).astype(dtype))
    self.sin_cached = Buffer(jnp.sin(emb).astype(dtype))
...
class DynamicNTKScalingRotaryEmbedding(RotaryEmbedding):
  ...
  def _set_cos_sin_cache(self, seq_len, dtype):
    ...
    if seq_len > self.max_position_embeddings:
      ...
      inv_freq: jax.Array = 1.0 / (
        base ** (jnp.arange(0, self.dim, 2) / self.dim)
      )
      self.inv_freq = Buffer(inv_freq)
...
def yarn_find_correction_dim(
  num_rotations: int, dim: int, base=10000, max_position_embeddings=2048
):
  return (
...
class MLP(nn.Module):
  def __init__(
    ...
    *,
    ...
    self.act_fn: Callable[[jax.Array], jax.Array] = getattr(
      nnx, config.hidden_act
    )
